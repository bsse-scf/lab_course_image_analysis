...
DATASETS: dict[str, Dataset] = {
    "bbbc020": Dataset(
        ...
        # Controls are spelled "Kontrolle" in the source data; the schedule's
...
MISC_PROVENANCE = {
    ...
    "artifacts": "derived: BBBC020 Kontrolle2 with one processing artifact each (.course/scripts/make_derived_data.py)",
}


HEADER = """# Data provenance

Generated by `.course/scripts/fetch_data.py` - do not edit by hand.

Every file below is redistributed from the Broad Bioimage Benchmark Collection
under CC BY 3.0, border-cropped (see .course/scripts/fetch_data.py) and downscaled so
...
def write_manifest(entries: dict[str, list[Path]]) -> None:
    ...
    if local:
        ...
        lines.append(
            ...
            "datasets above by `.course/scripts/make_derived_data.py`. All at most 512 px on "
            "the longest edge.\n"
        )
        lines.append("| file | source | accession | licence | transform | sha256 |")
        lines.append("|---|---|---|---|---|---|")
